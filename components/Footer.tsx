    ...
    <footer className="border-t border-slate-200 bg-brand-navy text-white">
      <div className="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
        <div className="grid gap-7 sm:grid-cols-2 sm:gap-10 lg:grid-cols-[1.3fr_repeat(3,1fr)]">
          <div className="grid grid-cols-[auto_1fr] items-start gap-x-4 gap-y-3 sm:block"><Logo className="rounded-xl bg-white px-2" /><p className="text-sm leading-5 text-white/70 sm:mt-4 sm:max-w-xs sm:leading-6">Tienda oficial de Exploradores del Rey en Panamá. Uniformes y accesorios para destacamentos.</p><div id="redes-sociales" className="col-span-2 sm:mt-5"><p className="sr-only sm:not-sr-only sm:mb-3 sm:text-xs sm:font-bold sm:uppercase sm:tracking-widest sm:text-white/50">Redes sociales</p><SocialMedia /><p className="sr-only sm:not-sr-only sm:mt-2 sm:text-xs sm:text-white/50">Enlaces oficiales pendientes de confirmar.</p></div></div>
        ...
        <div className="mt-6 border-t border-white/15 pt-4 text-xs text-white/60 sm:mt-10 sm:flex sm:items-center sm:justify-between sm:gap-2 sm:pt-6 sm:text-sm"><p>© {new Date().getFullYear()} Exploradores del Rey Panamá.</p><p className="mt-1 sm:mt-0">Tienda oficial.</p></div>
      </div>
    </footer>
  );
}
