...
export default function SearchBar() {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger aria-label="Buscar productos" className="flex h-11 w-11 items-center justify-center rounded-xl text-brand-navy transition hover:bg-blue-50 hover:text-brand-blue"><Search aria-hidden="true" className="h-5 w-5" /></DialogTrigger>
      <DialogContent className="flex max-h-[88vh] max-w-3xl flex-col overflow-hidden bg-white p-5 sm:p-6">
        ...
        <form className="relative" onSubmit={(event) => event.preventDefault()} role="search">
          ...
          {search && <button type="button" onClick={() => setSearch("")} aria-label="Limpiar búsqueda" className="absolute right-1 top-0 flex h-12 w-11 items-center justify-center text-slate-500 hover:text-brand-red"><X aria-hidden="true" className="h-5 w-5" /></button>}
        </form>
        <div className="min-h-40 flex-1 overflow-y-auto rounded-xl border border-slate-200" aria-live="polite">
          {loading ? <div role="status" className="flex items-center justify-center gap-2 py-12 font-bold text-brand-blue"><Loader2 aria-hidden="true" className="h-5 w-5 animate-spin" /> Buscando…</div>
