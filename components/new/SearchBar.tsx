      ...
      <DialogContent className="flex max-h-[88vh] max-w-3xl flex-col overflow-hidden bg-white p-5 sm:p-6">
        ...
        <div className="min-h-40 flex-1 overflow-y-auto rounded-xl border border-slate-200" aria-live="polite">
          ...
          : products.length ? <ul className="divide-y divide-slate-100">{products.map((product) => {
            const image = product.images?.[0]?.asset ? urlFor(product.images[0]).width(240).height(240).fit("max").auto("format").url() : "/product-placeholder.svg";
            return <li key={product._id}><Link href={`/product/${product.slug?.current}`} onClick={() => setOpen(false)} className="flex min-h-24 items-center gap-4 p-3 transition hover:bg-blue-50"><span className="relative h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-slate-100"><Image src={image} fill sizes="80px" className="object-contain p-1" alt={product.name ?? "Producto"} /></span><span className="min-w-0 flex-1"><span className="line-clamp-2 font-bold text-brand-navy">{product.name}</span><PriceView price={product.price} discount={product.discount} className="mt-1 text-base" /></span></Link></li>;
          })}</ul>
