...
export default function Sidebar({ isOpen, onClose, categories }: SidebarProps) {
  ...
  return createPortal(
    <div className={`fixed inset-0 z-[100] transition ${isOpen ? "visible bg-brand-navy/60 opacity-100" : "invisible opacity-0"}`} aria-hidden={!isOpen}>
      <div id="mobile-navigation" ref={sidebarRef} role="dialog" aria-modal="true" aria-label="Menú principal" className={`h-full w-[min(88vw,360px)] overflow-y-auto bg-white p-5 shadow-2xl transition-transform ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex items-center justify-between border-b border-slate-200 pb-4"><Logo /><button onClick={onClose} aria-label="Cerrar menú" className="flex h-11 w-11 items-center justify-center rounded-xl text-brand-navy hover:bg-red-50 hover:text-brand-red"><X aria-hidden="true" /></button></div>
