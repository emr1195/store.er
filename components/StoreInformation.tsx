    ...
    <section aria-labelledby="store-info-title" className="mx-auto max-w-screen-xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <div className="grid gap-8 lg:grid-cols-[.72fr_1.28fr]">
          ...
          <div className="grid gap-4 sm:grid-cols-2">{details.map(({ icon: Icon, label, value }) => <article key={label} className="rounded-2xl bg-page-bg p-5"><Icon aria-hidden="true" className="h-5 w-5 text-brand-blue" /><h3 className="mt-3 font-bold text-brand-navy">{label}</h3><p className="mt-1 text-sm leading-6 text-slate-600">{value}</p></article>)}</div>
        </div>
      </div>
    </section>
  );
